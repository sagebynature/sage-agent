"""Tests for central configuration loading, resolution, and merge."""
...
from sage.central_config import (
    AgentOverrides,
    CentralConfig,
    ConfigOverrides,
    load_central_config,
...
class TestCentralConfig:
    ...
    def test_extra_fields_rejected(self) -> None:
        with pytest.raises(Exception):
            CentralConfig(unknown_section="value")  # type: ignore[call-arg]


# ---------------------------------------------------------------------------
# resolve_central_config_path tests
# ---------------------------------------------------------------------------


class TestResolveCentralConfigPath:
    def test_cli_path_found(self, tmp_path: Path) -> None:
        toml_path = tmp_path / "config.toml"
        toml_path.write_text("[defaults]\n")
        assert resolve_central_config_path(str(toml_path)) == toml_path

    def test_cli_path_not_found_raises(self, tmp_path: Path) -> None:
        with pytest.raises(ConfigError, match="Central config not found"):
            resolve_central_config_path(str(tmp_path / "nonexistent.toml"))
    ...
    def test_env_var_found(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert resolve_central_config_path() == toml_path
...
# ---------------------------------------------------------------------------
# load_central_config tests
# ---------------------------------------------------------------------------


class TestLoadCentralConfig:
    def test_none_returns_none(self) -> None:
        assert load_central_config(None) is None
    ...
    def test_loads_minimal_toml(self, tmp_path: Path) -> None:
        ...
        cfg = load_central_config(toml_path)
...
""",
    ...
    def test_unknown_keys_rejected(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ConfigError, match="Invalid central config"):
            load_central_config(toml_path)
...
# ---------------------------------------------------------------------------
# Integration: load_config with central config
# ---------------------------------------------------------------------------
...
class TestLoadConfigWithCentral:
    ...
    def test_no_model_anywhere_raises(self, tmp_path: Path) -> None:
        """ConfigError when no model in frontmatter or central config."""
        cfg_path = _write_md(
            tmp_path / "AGENTS.md",
            {"name": "agent", "description": "No model here"},
        )
        central = CentralConfig()  # no defaults
    ...
    def test_no_model_without_central_raises(self, tmp_path: Path) -> None:
        """ConfigError when no model and no central config."""
        cfg_path = _write_md(
            tmp_path / "AGENTS.md",
            {"name": "agent"},
        )
    ...
    def test_backward_compat_no_central(self, tmp_path: Path) -> None:
        """Existing configs with model work without central config."""
        cfg_path = _write_md(
            tmp_path / "AGENTS.md",
    ...
    def test_config_ref_subagent_merged_with_central(self, tmp_path: Path) -> None:
        """Config-ref subagents are merged with central config."""
        _write_md(
            tmp_path / "sub.md",
